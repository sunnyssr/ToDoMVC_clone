...
let dateElement = document.querySelector(".select-date");
...
let showTodos = (todoListElement, filteredArray) => {
  ...
  let timeSelected = new Date(dateElement.value);
  filteredArray = filteredArray.filter((todo) => ((timeSelected.getFullYear() === (new Date(todo.date)).getFullYear())&&(timeSelected.getMonth() === (new Date(todo.date)).getMonth())&&(timeSelected.getDate() === (new Date(todo.date)).getDate() ) ) )
...
let refreshToDoList = () =>{
  ...
  let timeSelected = new Date(dateElement.value);
  let filteredArray = getActiveTodos();
  filteredArray = filteredArray.filter((todo) => ((timeSelected.getFullYear() === (new Date(todo.date)).getFullYear())&&(timeSelected.getMonth() === (new Date(todo.date)).getMonth())&&(timeSelected.getDate() === (new Date(todo.date)).getDate() ) ) )
  pendingCounter.innerText = filteredArray.length;
...
dateElement.addEventListener("change", refreshToDoList);

let updateDate = () => {
  var today = new Date();
  dateElement.value = today.getFullYear() + '-' + ('0' + (today.getMonth() + 1)).slice(-2) + '-' + ('0' + today.getDate()).slice(-2);
}
updateDate();
refreshToDoList();
